    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.orders.update', $order->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            ...
            @method('PUT')
                ...
                <input type="datetime-local" name="delivery_time" value="{{ old('delivery_time', $order->delivery_time ? $order->delivery_time->format('Y-m-d\TH:i') : '') }}" class="w-full border border-gray-300 rounded p-2">
                @error('delivery_time')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
