    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.items.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
                ...
                <input type="text" name="barcode" value="{{ old('barcode') }}" class="w-full border border-gray-300 rounded p-2">
                @error('barcode')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <select name="status_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($statuses as $option)
                        <option value="{{ $option->id }}" {{ old('status_id') == $option->id ? 'selected' : '' }}>{{ $option->name }}</option>
                    @endforeach
                </select>
                @error('status_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
